...
def preprocess_clip_bgr_to_btchw_fp32(frames_bgr, out_w=224, out_h=224, norm="imagenet") -> np.ndarray:
    xs = []
    for f in frames_bgr:
        ...
        x = np.transpose(x, (2, 0, 1))  # CHW
        xs.append(x)
    x = np.stack(xs, axis=0)      # (T,3,224,224)
    x = np.expand_dims(x, axis=0) # (1,T,3,224,224)
...
def main():
    ...
    # Classifier (D5) optional wiring
    ap.add_argument("--cls-onnx", default=None, help="(optional) video classifier ONNX path (expects 1xT x3x224x224)")
    ap.add_argument("--cls-t", type=int, default=16, help="clip length T")
    ap.add_argument("--cls-every", type=int, default=5, help="run classifier every N processed frames when event active")
